...
function goNext(){if(s.status!=='win')return;if(window.parent!==window){window.parent.postMessage({type:'golden-scapegoat-open-next',level:C.level},MESSAGE_ORIGIN);return;}location.href=C.level>=12?'../index.html':(C.next||`level-${C.level+1}.html`)}function continueAfterWin(){if(C.echoTutorial===true&&!hasSeenEchoTutorial())markEchoTutorialSeen();goNext()}
document.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.repeat&&s.status==='win'&&!document.activeElement?.matches('input,textarea,select,video')){e.preventDefault();continueAfterWin();return}if(DIR[e.key]){e.preventDefault();move(e.key)}if(e.key==='r'||e.key==='R')reset();if(e.key==='z'||e.key==='Z')undo();if(e.key==='h'||e.key==='H')hint()});
