...
const VIEW_W=1420,VIEW_H=835,DPR=Math.min(window.devicePixelRatio||1,2);cv.width=VIEW_W*DPR;cv.height=VIEW_H*DPR;cv.style.width=VIEW_W+'px';cv.style.height=VIEW_H+'px';x.setTransform(DPR,0,0,DPR,0,0);
...
function notifyComplete(){if(window.parent!==window)window.parent.postMessage({type:'golden-scapegoat-complete',level:C.level},window.location.origin)}
...
function goNext(){if(s.status!=='win')return;if(window.parent!==window){window.parent.postMessage({type:'golden-scapegoat-open-next',level:C.level},window.location.origin);return;}location.href=C.next||'../index.html'}
