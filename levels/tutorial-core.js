...
let s=fresh();let echoTutorialOpen=false;
...
function reset(){echoTutorialOpen=false;s=fresh();showHintBar(null);draw()}
...
const ECHO_TUTORIAL_KEY='golden-scapegoat:echo-tutorial-v1';function hasSeenEchoTutorial(){try{return localStorage.getItem(ECHO_TUTORIAL_KEY)==='1'}catch(_){return false}}function markEchoTutorialSeen(){try{localStorage.setItem(ECHO_TUTORIAL_KEY,'1')}catch(_){}}function setEchoTutorialVideo(playing){const video=document.querySelector('#echoTutorialVideo');if(!video)return;if(playing)video.play().catch(()=>{});else video.pause()}function needsEchoTutorial(){return C.level===5&&C.echoTutorial===true&&!hasSeenEchoTutorial()}function updateWinModal(){const won=s.status==='win',modal=document.querySelector('#winModal'),echoModal=document.querySelector('#echoTutorialModal'),showEchoTutorial=won&&echoTutorialOpen&&needsEchoTutorial();if(modal){modal.hidden=!won||showEchoTutorial;modal.setAttribute('aria-hidden',String(!won||showEchoTutorial));}if(won){const title=document.querySelector('#winTitle'),story=document.querySelector('#winStory');if(title)title.textContent=C.winTitle||'恭喜通关';if(story)story.innerHTML=C.winStory||'';document.querySelector('#winSteps').textContent=`本关共使用 ${s.step} 步`;}if(echoModal){echoModal.hidden=!showEchoTutorial;echoModal.setAttribute('aria-hidden',String(!showEchoTutorial));setEchoTutorialVideo(showEchoTutorial);}}
...
function goNext(){if(s.status!=='win')return;if(needsEchoTutorial()&&!echoTutorialOpen){echoTutorialOpen=true;draw();return;}if(window.parent!==window){window.parent.postMessage({type:'golden-scapegoat-open-next',level:C.level},MESSAGE_ORIGIN);return;}location.href=C.level>=12?'../index.html':(C.next||`level-${C.level+1}.html`)}
function continueAfterWin(){if(echoTutorialOpen&&needsEchoTutorial())markEchoTutorialSeen();goNext()}
document.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.repeat&&s.status==='win'&&!document.activeElement?.matches('input,textarea,select,video')){e.preventDefault();if(echoTutorialOpen)continueAfterWin();else goNext();return}if(DIR[e.key]){e.preventDefault();move(e.key)}if(e.key==='r'||e.key==='R')reset();if(e.key==='z'||e.key==='Z')undo();if(e.key==='h'||e.key==='H')hint()});
document.querySelector('#undo').onclick=undo;document.querySelector('#hint').onclick=hint;document.querySelector('#reset').onclick=reset;document.querySelector('#winRestart').onclick=reset;document.querySelector('#winNext').onclick=goNext;const echoTutorialContinue=document.querySelector('#echoTutorialContinue');if(echoTutorialContinue)echoTutorialContinue.onclick=continueAfterWin;fu.onload=draw;verify();draw();
})();
